...
function longestSentence (string) {
  ...
  for (let idx = 0; idx < string.length; idx++) {
    let start = 0; 
    if(endings.includes(string[idx])) {
      substrings.push(string.substring(start, idx + 1)); 
      start = idx + 1; 
    }
  }

  let longest = substrings.sort((a, b) => a.length - b.length)[0]; 

  let returnStatement = `${longest} -- The longest sentence has ${longest.split(' ').length} words.`; 
