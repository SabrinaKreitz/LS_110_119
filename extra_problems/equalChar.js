...
A: Identify the length of the longest possible substring between two equal characters in a string 

CREATE `count` variable and set to -1
ITERATE over input string start at 0-indexed element
  - for each char1
    - ITERATE over input string starting at char2 (char1 + 1)
      - IF index char1 of input string is equal to index char2 of input string 
        - CREATE substring starting at char1 + 1 ending at char2 (excluding char2)
        - IF length of substring is greater than `count` 
          - REASSIGN count to length of substring 
RETURN `count` 
*/
let p = console.log; 
...
function maxLengthBetweenEqualCharacters (string) {
  let count = -1; 
  for (let char1 = 0; char1 < string.length - 1; char1++) {
    for (let char2 = char1 + 1; char2 < string.length - 1; char2++) {
      if(string[char1] === string[char2] && string.substring(char1 + 1, char2).length > count) {
        count = string.substring(char1 + 1, char2).length;
      }
    }
  }
  return count; 
